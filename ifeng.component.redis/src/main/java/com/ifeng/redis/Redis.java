...
import com.ifeng.configurable.redis.RedisConfig;
import com.ifeng.configurable.redis.RedisConfigManage;
...
public class Redis {
	...
	public Redis(String path){this.path = path;}
	 ...
	 * @return
	 */
	public JedisPool getJedisPool(String instanceName) throws Exception {
		initConfigs();
		if (jedisPool == null) {
	 ...
	 * @return
	 */
	public ShardedJedisPool getShardedJedisPool(String shardName) throws Exception {
		initConfigs();
	...
	private void initRedisConfigs() throws Exception {
		if (redisConfigs == null) {
			if (null == path || "".equals(path))
				...
				redisConfigs = new RedisConfigManage(path+"/conf/redis.properties").getRedisConfigs();
			}
		}
	}
	...
	private void initRedisPools() throws Exception {
		...
		for (Entry<String, JedisPoolConfig> config : redisPoolConfigs.entrySet()) {
			...
			pool = new JedisPool(config.getValue(), c1.getServerIp(), c1.getPort(),5*1000,c1.getPassword());
	...
	private void initShardRedisPools() throws Exception {
		...
		for (Entry<String, List<RedisConfig>> config : sharedRedisPoolConfigs.entrySet()) {
			...
			for (RedisConfig redisConfig : list) {
				...
				en.setPassword(redisConfig.getPassword());
				shardInfos.add(en);
			}
